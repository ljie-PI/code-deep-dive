...
| **IAgentProvider** | AI Provider 接口（`@archon/providers`），`ClaudeProvider`、`CodexProvider`、`PIProvider` 实现此接口；v0.3.x 取代了旧版位于 `@archon/core` 的 `IAssistantClient` |
...
| **IsolationResolver** | 6 步隔离解析引擎（v0.3.x 由 7 步精简；硬编码的 `maxWorktrees=25` 容量步骤已外移成可选 `cleanup.makeRoom()` 注入） |
...
| **Provider Registry** | `@archon/providers/registry.ts` 中的 Provider 注册表，启动时由 `registerBuiltinProviders()` 装填 |
...
| **WorkflowDeps** | 工作流引擎的依赖注入接口，包含 store、AI Provider 工厂（来自 `@archon/providers/registry`）、配置加载器 |
